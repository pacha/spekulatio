...
```
$ spekulatio build /path/to/input-dir -o /path/to/output-dir
```
...
* `RenderTemplate`: use a Jinja template and pass the input file as content to it.
...
actions:
  - name: Copy
    patterns:
      - "*.jpg"
      - "*.jpeg"
    output_name "{{ _input_path.with_suffix(".jpeg").name }}"
```
...
By {{ author }}.
```
The `Render`, `RenderTemplate` and `Md2Html` actions detect if the input file
has a front matter. If the front matter is present, the content is included in
the values associated to the file. The front matter is never included in the
final output file.
```
actions:
  - name: Render
...
Any file in any of the input directories can be used as template.
...
add an empty file named `.spekulatiokeep` in the input and add assign a `Noop`
action to it.
...
`get` function. It takes as a parameter the path to the corresponding file in
the input directory. For example, if `input-directory/dir1/file.md` generates
`output-directory/dir1/file.html`, then you need to call `_root.get("/dir1/file.md")`
...
`_node.get("dir/file.md")`, then the retrieved node will be the corresponding to the
`/some/dir/file.html` file.

Nodes have a number of useful attributes:

* `_node.sorted_children`: list of this node's children. It is an empty list for
  non-directory nodes.
* `_node.url`: URL of the node if the output directory was being served. The
  `_url_prefix` variable can be used to append a specific prefix to each URL.
* `_node.values`: dictionary with all the values defined and inherited by the
  node.

In the example above, we use this snippet to generate a list of links to the
game pages:
```
{% for node in _root.get("/games").sorted_children %}
<li>
    <a href="{{ node.url }}">{{ node.values.title }}</a>
</li>
{% endfor %}
